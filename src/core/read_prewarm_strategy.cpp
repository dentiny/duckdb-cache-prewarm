...
public:
	ReadBlockGroupTask(TaskExecutor &executor, BlockManager &block_manager_p, BufferManager &buffer_manager_p,
	                   block_id_t first_block_p, idx_t block_count_p, std::atomic<idx_t> &blocks_read_p)
	    : BaseExecutorTask(executor), block_manager(block_manager_p), buffer_manager(buffer_manager_p),
	      first_block(first_block_p), block_count(block_count_p), blocks_read(blocks_read_p) {
	}
...
idx_t ReadPrewarmStrategy::Execute(DuckTableEntry &table_entry, const unordered_set<block_id_t> &block_ids) {
	...
	for (size_t i = 0; i < unloaded_handles.size();) {
		...
		for (idx_t offset = 0; offset < block_count; offset += blocks_per_task) {
			...
			if (use_parallel) {
				auto task =
				    make_uniq<ReadBlockGroupTask>(executor, block_manager, buffer_manager, task_first_block,
